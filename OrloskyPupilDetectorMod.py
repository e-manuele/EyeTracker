...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame,
    ...
    for i in range(1, 4):
        ...
        if len(reduced_contours) > 0 and len(reduced_contours[0]) > 5:
            ...
            if debug_mode_on:
                cv2.putText(gray_copies[i - 1], "%filled:     " + str(current_goodness[0])[
                                                                  :5] + " (percentage of filled contour pixels inside ellipse)",
                            (10, 30), font, .55, (255, 255, 255), 1)  # %filled
                cv2.putText(gray_copies[i - 1],
                            "abs. pix:   " + str(total_pixels[0]) + " (total pixels under fit ellipse)", (10, 50), font,
                            .55, (255, 255, 255), 1)  # abs pix
                cv2.putText(gray_copies[i - 1], "pix ratio:  " + str(
                    total_pixels[1]) + " (total pix under fit ellipse / contour border pix)", (10, 70), font, .55,
                            (255, 255, 255), 1)  # abs pix
                cv2.putText(gray_copies[i - 1], "final:     " + str(final_goodness) + " (filled*ratio)", (10, 90), font,
                            .55, (255, 255, 255), 1)  # skewedness
    ...
    if final_contours and not isinstance(final_contours[0], list) and len(final_contours[0] > 5):
        ...
        cv2.putText(test_frame, "SPACE = play/pause", (10, 410), cv2.FONT_HERSHEY_SIMPLEX, .55, (255, 90, 30),
                    2)  # space
        cv2.putText(test_frame, "Q      = quit", (10, 430), cv2.FONT_HERSHEY_SIMPLEX, .55, (255, 90, 30), 2)  # quit
        cv2.putText(test_frame, "D      = show debug", (10, 450), cv2.FONT_HERSHEY_SIMPLEX, .55, (255, 90, 30),
                    2)  # debug
...
def process_frame(frame):
    ...
    final_rotated_rect = process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed,
...
def process_video(video_path, input_method):
    ...
    while True:
        ...
        # SCRITTURA FRAME (QUESTO MANCAVA)
        out.write(frame)
...
def process_video_with_log(video_path, input_method):
    ...
    # Leggi primo frame per inizializzare writer
    ret, frame = cap.read()
    if not ret:
        print("Error: Could not read first frame.")
        return
    ...
    out = cv2.VideoWriter(
        os.path.join(os.getcwd(), 'output_video.mp4'),
        fourcc,
        fps,
        (640, 480)
    )
    ...
    # CSV log
    log_path = os.path.join(os.getcwd(), 'eye_log.csv')
    log_file = open(log_path, 'w', newline='')
    csv_writer = csv.writer(log_file)
    csv_writer.writerow(['second', 'x', 'y', 'angle'])
    ...
    while True:
        ...
        if debug_mode_on:
            ...
            cv2.circle(debug_frame, darkest_point, 10, (0,0,255), -1)
            cv2.imshow('Darkest Image', debug_frame)
        ...
        # Process frame e ottieni ellisse
        pupil_rotated_rect = process_frames(
            thresholded_image_strict,
            thresholded_image_medium,
            thresholded_image_relaxed,
        ...
        # Disegna ellisse sul frame
        cv2.ellipse(frame, pupil_rotated_rect, (0, 255, 0), 2)

        # Scrivi frame nel video
        out.write(frame)

        # Scrivi dati nel CSV
        center = pupil_rotated_rect[0]  # (x, y)
        angle = pupil_rotated_rect[2]   # angolo
        csv_writer.writerow([frame_number / fps, center[0], center[1], angle])
    ...
    print(f"Video salvato in: {os.path.join(os.getcwd(), 'output_video.avi')}")
    print(f"Log salvato in: {log_path}")
def process_video_only_log(video_path, input_method):
    ...
    # Leggi primo frame per inizializzare writer e centro di riferimento
    ret, frame = cap.read()
    if not ret:
        print("Error: Could not read first frame.")
        return
    ...
    first_pupil = process_frame(frame)
    center_ref = first_pupil[0]  # centro iniziale (x, y)
    ...
    log_file = open(log_path, 'w', newline='')
    csv_writer = csv.writer(log_file)
    csv_writer.writerow(['second', 'dx', 'dy', 'angle', 'direction'])
    ...
    while True:
        ...
        # Trova ellisse pupilla
        pupil_rotated_rect = process_frame(frame)
        center = pupil_rotated_rect[0]
        angle = pupil_rotated_rect[2]

        # Coordinate relative
        dx = center[0] - center_ref[0]
        dy = center[1] - center_ref[1]

        # Stima direzione degli occhi
        threshold = 5  # tolleranza in pixel per considerare "centrale"
        if dx > threshold:
            dir_x = "destra"
        elif dx < -threshold:
            dir_x = "sinistra"
        ...
        direction = f"{dir_y}-{dir_x}"  # esempio: "alto-destra"
        ...
        # Scrivi dati su CSV
        csv_writer.writerow([frame_number / fps, dx, dy, angle, direction])
        frame_number += 1
...
def select_video():
    ...
    base_url = os.getcwd()
    video_path = base_url + '/eye_test.mp4'
    ...
    process_video_only_log(video_path, 1)
