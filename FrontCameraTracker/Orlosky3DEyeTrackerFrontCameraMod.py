...
def process_camera():
    ...
    eye_cap.set(cv2.CAP_PROP_FRAME_WIDTH, EYE_WIDTH)
    eye_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, EYE_HEIGHT)
    ...
    if external_cap.isOpened():
        ...
        external_cap.set(cv2.CAP_PROP_FPS, FPS)
        print(f"External camera opened ({EXT_WIDTH}x{EXT_HEIGHT})")
    else:
        print("Warning: Could not open external camera.")
        external_cap = None
    ...
    # ---- Output directory and video writers ----
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    OUTPUT_DIR = os.path.join(BASE_DIR, "FrontCameraTracker")
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    eye_path = os.path.join(OUTPUT_DIR, "eye_tracking_video.mp4")
    external_path = os.path.join(OUTPUT_DIR, "external_tracking_video.mp4")

    eye_writer = cv2.VideoWriter(eye_path, fourcc, FPS, (EYE_WIDTH, EYE_HEIGHT))
    if not eye_writer.isOpened():
        print("ERROR: eye_writer not opened")
        return

    external_writer = None
    if external_cap is not None:
        external_writer = cv2.VideoWriter(external_path, fourcc, FPS, (EXT_WIDTH, EXT_HEIGHT))
        if not external_writer.isOpened():
            print("ERROR: external_writer not opened")
            external_writer = None

    print("Saving videos in:", OUTPUT_DIR)

    # ---- Main loop ----
    while True:
        ret_eye, eye_frame = eye_cap.read()
        if not ret_eye:
            print("Eye camera frame not available.")
            break

        if external_cap is not None:
            ret_ext, external_frame = external_cap.read()
            if not ret_ext:
                external_frame = np.zeros((EXT_HEIGHT, EXT_WIDTH, 3), dtype=np.uint8)

        # Process pupil tracking and gaze vector
        final_rotated_rect = process_frame(eye_frame)

        # Write to video files
        eye_writer.write(eye_frame)
        if external_cap is not None and external_writer is not None:
            external_writer.write(external_frame)

        # Show live feedback
        cv2.imshow("Frame with Ellipse and Rays", eye_frame)
        if external_cap is not None:
        ...
        if key == ord('q'):
            ...
            # Manual calibration
            on_mouse_frame_with_rays(cv2.EVENT_LBUTTONDOWN, circle_x, circle_y, None, None)
        elif key == ord('r'):
            calibrated = False
    ...
    print("Videos saved and cameras released.")
...
def process_video():
    ...
    EYE_WIDTH = 640
    EYE_HEIGHT = 480
    FPS = int(eye_cap.get(cv2.CAP_PROP_FPS)) or 30

    # ---- Optional external camera preview ----
    external_cap = cv2.VideoCapture(1, cv2.CAP_MSMF)
    if external_cap.isOpened():
        external_cap.set(cv2.CAP_PROP_FRAME_WIDTH, EXT_WIDTH)
        external_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, EXT_HEIGHT)
    ...
    # ---- Main loop ----
    while True:
        ret_eye, frame_eye = eye_cap.read()
        ret_ext, frame_ext = (True, None)
        if external_cap is not None:
            ret_ext, frame_ext = external_cap.read()
        ...
        # ---- Process eye frame ----
        final_rotated_rect = process_frame(frame_eye)

        # ---- Update gaze circle for external camera ----
        update_gaze_circle_from_current_gaze()

        # ---- Draw circle on external frame ----
        if frame_ext is not None:
            cv2.circle(frame_ext, (circle_x, circle_y), 15, (0, 255, 0), 3)

        # ---- Show frames ----
        cv2.imshow("Frame with Ellipse and Rays", frame_eye)
        if frame_ext is not None:
            cv2.imshow("External Camera", frame_ext)

        # ---- Write to video ----
        eye_writer.write(frame_eye)
        if frame_ext is not None and external_writer is not None:
            external_writer.write(frame_ext)
